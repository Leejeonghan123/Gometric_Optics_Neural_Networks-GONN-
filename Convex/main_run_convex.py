...
class Ray_solver:
    ...
    def Ray_Plot(self, save_file=None):
        ...
        if save_file:  
            np.savetxt('Convex/Pred_/'+ save_file, self.ray12, delimiter=',')
...
angle = np.linspace(np.pi/2., 3*np.pi/2, 100)        
...
for name, data in zip(filenames, data_lists):
    np.savetxt(f'Convex/{name}.txt', data, delimiter=',')
